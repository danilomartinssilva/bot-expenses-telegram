...
export class TelegramResponder implements BotResponder {
  ...
  async ack(text?: string): Promise<void> {
    await this.ctx.answerCbQuery(text);
  }

  private responsibleKeyboard() {
  ...
  async askResponsible(): Promise<void> {
    await this.ctx.reply('Quem é o responsável por essa despesa?', this.responsibleKeyboard());
  }
  ...
  async askValueEdit(): Promise<void> {
    await this.ctx.reply('Digite o valor correto. Exemplo: 42,90');
  }

  async askDescriptionEdit(): Promise<void> {
    await this.ctx.reply('Digite a nova descrição.\nExemplo: Mercado Extra');
  }
  ...
  async showPreview(expense: Expense): Promise<void> {
    await this.ctx.reply(this.buildPreview(expense), this.confirmationKeyboard());
  }
}
